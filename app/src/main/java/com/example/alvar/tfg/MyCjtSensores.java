...
public class MyCjtSensores {
    ...
    public String[] getNombresBySala(String sala)
    {
        ...
        for(int i=0;i<ListSensors.size();++i) {
            if(ListSensors.get(i).getUbicacion().equals(sala)) {
                values[j] = ListSensors.get(i).getNombre();
                ++j;
            }
    ...
    public void ActualizarDispositivo(String id, Double temp, Double hum)
    {
    ...
    public String actuatorIdBySala(String sala, String name)
    {
        String idreturn = " ";
        for(int i=0;i<ListSensors.size();++i) {
            if(ListSensors.get(i).getNombre().equals(name) && ListSensors.get(i).getUbicacion().equals(sala))
